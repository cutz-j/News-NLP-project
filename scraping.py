...
def hani_scrape(word_list):
    ...
    for word in word_list: # 관련 단어 모두 검색
        ...
        url += parse.quote(word)
        url += "&media=news&sort=d&period=all&datefrom=2013.01.01&dateto=2018.11.18&pageseq=0"
        # selenium drvier (chromedriver download)
        html = requests.get(url)
        soup = BeautifulSoup(html.text, 'html.parser')
        # 총 출력 페이지 parsing #
        ...
        for i in range(1, total_page_num): # 총 출력 페이지만큼 페이지 이동
            ...
            for res in results: # 해당 페이지의 기사 10건 접속
                ...
                try:
                    results_art = results_art[0].text
                    results_art = results_art[:-results_art[::-1].index(".다")]
                    results_art = results_art.strip()
                    ...
                    if "@hani.co.kr" in results_art:
                        reg = re.compile("[\w\W]{3,5}[a-zA-Z]+@hani.co.kr")
...
if __name__ == "__main__":
    ## parameter ##
    word_list = ['원전', '신고리', '원자력']
    res_list_1 = joongang_scrape(word_list)
#    res_list_2 = hani_scrape(word_list)
    res_1 = pd.DataFrame(res_list_1).drop_duplicates()
    res_1.to_excel("d:/news_project/res1_test.xlsx", header=False, index=False, encoding='utf-8')
